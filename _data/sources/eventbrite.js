...
class eventbrite {
    constructor(token = process.env.EVENTBRITE_TOKEN) {
        ...
        this.event = (event) => {
            ...
            return new this.eventClass(event.name.text, event.url, event.venue, description, event.start.utc, event.end.utc, null, event.capacity, event.id_free, null, event.groupName, event.groupLink, "Eventbrite", false, event.online_event);
        }
    }
